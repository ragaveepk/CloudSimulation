...
/**
A simple cloudsim plus example which uses defined configurations that has single datacenter, 5 hosts 20 vms and 35 cloudlets.
this simulation is uses Vm Allocation Simple, VmSchedulerTimeShared, CloudletSchedulerSpaceShared and utilization model as UtilizationModelDynamic with utilization Ratio as 0.5
*/
...
object SimulationOne {
  ...
  def createDataCenter(): DatacenterSimple = {
    ...
    val hostList_new = (1 to conf.getInt( simulation_one + "." + "datacenter" + ".numOfHosts")).map(host => createHosts).toList
  ...
  /***
   * this functions creates list of virutal machines and its required pes and set Clouldlet Scheduler as Space shared
   */
  def createVms: util.List[Vm] = {
    ...
    val list = (1 to NUMOFVMS).map(vm => new VmSimple(1000, VM_PES).setCloudletScheduler(new CloudletSchedulerSpaceShared)).toList
    list.asJava
  }
  ...
  def createCloudlets: util.List[CloudletSimple] = {
    ...
    val list = (1 to NUMOFCLOUDLETS).map(c => new CloudletSimple(CLOUDLETLENGTH, NUMOFCLOUDLETPES, new UtilizationModelDynamic(0.75))).toList
    list.asJava
  }
